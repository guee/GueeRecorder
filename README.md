...
　　今天还和SimpleScreenRecorder简单对比了一下性能。虽然我的程序对图像的处理流程上要复杂得多，但在龙芯平台上我的程序效率仍然略高一点，但在x86 UOS上SimpleScreenRecorder的CPU占用率要比我的程序低几个百分点。因为两者使用相同的x264库文件，编码参数也尽量一致，性能差距就只发生在视频编码之前。证实了我以前的判断，我对图像的更复杂的处理流程本来增加了CPU消耗，但它对龙芯CPU没有任何优化，简单的颜色空间转换消耗的CPU时间反而更高。然后我在龙芯开发者社区看到Loongnix系统中有龙芯优化过的x264库文件，可惜作为Linux新手的我没能在Loongnix成功配置Qt的开发环境，暂时试验不了龙芯优化过的x264编码库有多少性能提升。</br>
